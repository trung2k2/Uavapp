...
import { decryptWithDatConBridge } from './datcon-bridge.js'
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('decrypt:datcon', async (_event, payload) => {
    const { filePath, outputDir, options } = payload || {}
    if (!filePath || typeof filePath !== 'string') throw new Error('filePath required')
    const base = path.basename(filePath)
    try {
      return await decryptWithDatConBridge(filePath, outputDir, options)
    } catch (bridgeError) {
      throw new Error(`Failed to decode ${base} with DatCon Java bridge. ${String(bridgeError?.message || bridgeError)}`)
    }
  })
